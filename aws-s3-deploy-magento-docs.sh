...
echo -e "Which environment do you want to deploy?"
  select environment in "personal" "staging"; do
    case $environment in
      ...
      break;;
      # For some reason probably related to using a bash script an IAM credentials, cannot upload to staging root; must specify at least one directory or script hangs
